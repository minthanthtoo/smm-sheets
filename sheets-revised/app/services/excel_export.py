...
def run_excel_regeneration(
    ...
    if include_map:
        cmd.extend(["--include-map", json.dumps(include_map)])
    if include_files_map:
        cmd.extend(["--include-files-map", json.dumps(include_files_map)])
...
def build_export_zip(out_dir: Path, region: str | None = None, regions: List[str] | None = None) -> Path:
    ...
    if regions:
        ...
        for r in regions:
            rdir = out_dir / r
            if not rdir.exists():
                raise HTTPException(status_code=404, detail=f"Region {r} not found in export")
            region_dirs.append(rdir)
        zip_name = f"excel_export_{regions[0]}.zip" if len(regions) == 1 else "excel_export_selected.zip"
    elif region:
