...
@router.get("/outlets/history_all")
def outlet_history_all(limit: int = 200, q: str | None = None):
    limit = max(1, min(int(limit), 1000))
    conn = get_conn()
    try:
        rows = conn.execute(
            """
            SELECT h.outlet_id, h.outlet_name_mm, h.outlet_name_en, h.outlet_type, h.category,
                   h.region_id, h.effective_from, h.effective_to, h.changed_by, h.changed_at
            FROM outlet_history h
        ...
        for r in rows:
            d = row_to_dict(r)
            if query:
                hay = f"{d.get('outlet_id','')} {d.get('outlet_name_mm','')} {d.get('outlet_name_en','')} {d.get('region_id','')} {d.get('outlet_type','')}".lower()
                if query not in hay:
                    continue
            out.append(d)
