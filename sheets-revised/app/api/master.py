...
@router.get("/townships/history_all")
def township_history_all(limit: int = 200, q: str | None = None):
    limit = max(1, min(int(limit), 1000))
    conn = get_conn()
    try:
        rows = conn.execute(
            """
            SELECT township_id, township_name, township_name_en, region_id,
                   effective_from, effective_to, created_at, created_by
            FROM township_history
        ...
        for r in rows:
            d = row_to_dict(r)
            if query:
                hay = f"{d.get('township_id','')} {d.get('township_name','')} {d.get('region_id','')}".lower()
                if query not in hay:
                    continue
            out.append(d)
...
@router.get("/routes/history_all")
def route_history_all(limit: int = 200, q: str | None = None):
    limit = max(1, min(int(limit), 1000))
    conn = get_conn()
    try:
        rows = conn.execute(
            """
            SELECT route_id, route_name, region_id, van_id, way_code, township_id,
                   effective_from, effective_to, created_at, created_by
            FROM route_history
            ORDER BY effective_from DESC
            LIMIT ?
        ...
        for r in rows:
            d = row_to_dict(r)
            if query:
                hay = f"{d.get('route_id','')} {d.get('route_name','')} {d.get('region_id','')} {d.get('van_id','')}".lower()
                if query not in hay:
                    continue
            out.append(d)
        return JSONResponse({"rows": out})
...
@router.get("/products/history_all")
def product_history_all(limit: int = 200, q: str | None = None):
    limit = max(1, min(int(limit), 1000))
    conn = get_conn()
    try:
        rows = conn.execute(
            """
            SELECT product_id, sales_price, pack_size, ml_per_bottle, unit_type, category,
                   effective_from, effective_to, created_at, created_by
        ...
        for r in rows:
            d = row_to_dict(r)
            if query:
                hay = f"{d.get('product_id','')} {d.get('sales_price','')} {d.get('pack_size','')} {d.get('category','')}".lower()
                if query not in hay:
                    continue
            out.append(d)
        return JSONResponse({"rows": out})
